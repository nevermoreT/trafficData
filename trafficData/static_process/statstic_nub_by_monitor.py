...
dir="/media/tjx/768C23788C2331D3/20170501_05/5"

beginDate=pd.to_datetime("2017/5/5 17:30:00")
endDate=pd.to_datetime("2017/5/5 23:30:00")
zeroTime=pd.to_datetime("2017/5/5")
...
while beginDate<=endDate:
    ...
    if  num%2==0:
        if num/2<10:
            pattern="rImport_ysb_201705050{0}[0-2][0-9]".format(num/2)
        else:
            pattern="rImport_ysb_20170505{0}[0-2][0-9]".format(num/2)
    else:
        if num/2<10:
            pattern="rImport_ysb_201705050{0}[3-5][0-9]".format(num/2)
        else:
            pattern="rImport_ysb_20170505{0}[3-5][0-9]".format(num/2)
    ...
    #print result
    result.to_csv("5_5/result{0}.csv".format(num),index=False)
